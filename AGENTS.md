...
- **F8 (Docker/producción):** torch/torchvision pinneados (GAP-PROD-02), `HEALTHCHECK` contra `/api/health`, `.dockerignore` (no copia `.env`/`data/`), `stop_grace_period: 30s` para el shutdown limpio, usuario no-root `appuser` en el Dockerfile (en compose corre como root para poder escribir `./data`), limits de recursos (6G RAM / 3.5 CPU), labels OCI para build reproducible, Traefik reverse proxy opcional (profile `with-traefik`), backup automatizado de `./data` con rotación.
